...
def analyze():
    """Enhanced predict endpoint with FastSAM segmentation"""
    ...
    logger.info("=" * 80)
    logger.info("🚀 ENHANCED PREDICT ENDPOINT CALLED WITH FASTSAM")
    logger.info("=" * 80)
    ...
    # FIXED: Complete try-except block
    try:
        ...
        # FIXED: Complete try-except for segmentation
        try:
        ...
        # STEP 4: PROCESS EACH LEAF - DISEASE DETECTION
        predictions = []
        leaf_severities = []
            ...
            try:
                ...
                if predicted_class and error_message is None:
                    ...
                    logger.info(f"   ✅ Leaf {idx}: {predicted_class} ({confidence:.1f}%) - "
                            f"Model: {severity}, Color: {color_severity_level} ({color_severity}%)")
        ...
        # STEP 5: SELECT BEST PREDICTION
        best = max(predictions, key=lambda x: x["confidence"])
        predicted_class = best["predicted_class"]
        confidence = best["confidence"]
        
        logger.info(f"🎯 Best prediction: {predicted_class} ({confidence:.1f}%)")
        ...
        # STEP 7: GET LAND AREA & LOCATION INFO
        ...
        # STEP 8: GET DISEASE INFO & CALCULATE DOSAGE
        disease_info = get_disease_info(predicted_class)
        if disease_info:
            import copy
        ...
        # STEP 9: SAVE DETECTION FOR CHATBOT
        try:
        ...
        # STEP 10: PREPARE & RENDER RESULTS
        template_vars = {
            # ===== IMAGE URLS =====
            'image_url': url_for('static', filename=f'uploads/{image_filename}'),
            ...
            # ===== DISEASE INFORMATION =====
            'result': disease_info,  # This contains the normalized data
        ...
        # ADD THIS DEBUG LOGGING RIGHT BEFORE render_template:
        logger.info("=" * 80)
        logger.info("📦 TEMPLATE VARIABLES FOR RENDERING")
        logger.info("=" * 80)
        logger.info(f"✅ Image URL: {template_vars.get('image_url')}")
        logger.info(f"✅ GradCAM: {bool(template_vars.get('gradcam_url'))}")
        logger.info(f"✅ Heatmap: {bool(template_vars.get('heatmap_url'))}")
        ...
        if disease_info:
            logger.info(f"✅ Result keys: {list(disease_info.keys())}")
            if 'pesticide' in disease_info:
                logger.info(f"  Chemical: {bool('chemical' in disease_info['pesticide'])}")
                logger.info(f"  Organic: {bool('organic' in disease_info['pesticide'])}")
                
                if 'chemical' in disease_info['pesticide']:
                    c = disease_info['pesticide']['chemical']
                    logger.info(f"  Chem Usage: {bool(c.get('usage'))} ({len(c.get('usage', ''))} chars)")
                    logger.info(f"  Chem Frequency: {bool(c.get('frequency'))} ({len(c.get('frequency', ''))} chars)")
                    logger.info(f"  Chem Safety: {bool(c.get('safety'))} ({len(c.get('safety', ''))} chars)")
                
                if 'organic' in disease_info['pesticide']:
                    o = disease_info['pesticide']['organic']
                    logger.info(f"  Org Usage: {bool(o.get('usage'))} ({len(o.get('usage', ''))} chars)")
                    logger.info(f"  Org Frequency: {bool(o.get('frequency'))} ({len(o.get('frequency', ''))} chars)")
                    logger.info(f"  Org Safety: {bool(o.get('safety'))} ({len(o.get('safety', ''))} chars)")
        else:
            logger.error("❌ NO DISEASE INFO!")
    ...
    # FIXED: Main exception handler for the entire function        
    except Exception as e:
